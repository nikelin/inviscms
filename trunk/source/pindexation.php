...
<html>
	...
	<body>
	<h2>Текущий репозиторий пакетов</h2>
	<?php
	...
	for($f=readdir($dir);($f!==false);$f=readdir($dir))
	{
		if($f!='..' && $f!='.')
		{
			$data=simplexml_load_file($path.'/'.$f.'/info.xml');
			...
			<div class='table' style='margin-bottom:40px;border:3px #000000 dashed;'>
				<h4 style='background-color:#EEEEEE;' onclick='Invis.tools.changeElVis("<?=$phash;?>","switch");return false;'><a name='<?=md5($f);?>'/>Package title: <input type='text' name='title' value='<?=($data->title)?$data->title:$f;?>'/>(status:<input type='text' name='status' style="color:<?=($status=="on")?"#00FF00":"FF0000";?>;" value='<?=$status;?>'/>) <?=($indexated)?'Alredy added to index':"Not indexed in database";?></h4>
				<div style='display:none' id='<?=$phash;?>'>
					...
					<div class='body'>
						...
						<div class='row'>
							...
							<div class='col'><input type='text' name='company' value='<?=$data->authority->company;?>'/></div>
						</div>
						<div class='row'>
							<div class='col'>Version:</div>
							<div class='col'><input type='text'  name='version' value='<?=$data->authority->version;?>'/></div>
						</div>
						<div class='row'>
							<div class='col'>InnoWeb RegCode:</div>
							<div class='col'><input type='text' name='regcode' value='<?=$data->authority->regcode;?>'/><button style='width:90px;font-weight:bold;font-size:13px;'><u>R</u>EQUEST</button></div>
						</div>
						<div class='row'>
							<div class='col'>Publication date:</div>
							<div class='col'><input type='text' name='pubdate' value='<?=$data->authority->pub_date;?>'/></div>
						</div>
						<div class='row'>
							<div class='col'>E-mail to support request:</div>
							<div class='col'><input type='text' name='reply_to' value='<?=$data->authority->reply_to;?>'/></div>
						</div>
						<div class='row'>
							<div class='col'>License:</div>
							<div class='col'><input type='text' name='license' value='<?=$data->authority->license;?>'/></div>
						</div>
						<div class='row'>
							<div class='col'>License URL address:</div>
							<div class='col'><input type='text' name='license_url' value='<?=$data->authority->license;?>'/></div>
						</div>
						<div class='row'>
							<div class='col' style='font-weight:bold;'>Dependencies:</div>
						</div>
						<?=ext_info_block($phash,$data->dependencies->abstractions,"abstractions","Abstracted classes:");?>
						<?=ext_info_block($phash,$data->dependencies->interfaces,"interfaces","Linked interfaces:");?>
						<?=ext_info_block($phash,$data->dependencies->extending,"extending","Parent classes:");?>
						<?=ext_info_block($phash,$data->dependencies->libs,"libs","Linked libraries:");?>
						<?=ext_info_block($phash,$data->dependencies->errors,"errors","Errors definitions:");?>
					<div class='row'>
