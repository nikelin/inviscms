...
class database implements databaseI{
	...
	public function proceedQuery($query){
		...
		if(trim($query)!=''){
			...
			if($this->isError()) new Exception($this->sqlErrorString());#die_r($this->sqlErrorString()); 
		}else{
			#$this->sqlErrorExpect(DATABASE_WRONG_PARAM,__LINE__,__FILE__);
		}
	...
	public function getNumrows($q){
		...
		if($q){
			...
			if($this->isError())$this->sqlErrorExpect(DATABASE_PROCEED_ERROR,__LINE__,__FILE__);
		}else{
			$this->sqlErrorExpect(DATABASE_WRONG_PARAM,__LINE__,__FILE__);
		}
		return $result;
