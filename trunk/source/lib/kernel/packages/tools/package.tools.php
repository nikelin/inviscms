...
class tools{
	...
	public function pageExists($id){
		return $GLOBALS['database']->checkRowExists("pages",array((is_string($indefier)?"ufu":"id")=>$indefier,'status'=>'on'));
	}


	public function getGoodID($hash,$hash_length)
	{
		return $GLOBALS['database']->getSQLParameter("catalog","id",array("LEFT(MD5(id),".$hash_length.")"=>$hash));
	}
	
	#
	# FIXME: refactoring needs to work with alphabet drivers
	#
