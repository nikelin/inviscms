...
class security{
	...
	public function renewEXTR($old)
	{
		...
		if($database->checkRowExists("users",array("extr"=>$old)))
		{
			...
			{
				$GLOBALS['attempts']->send("Обновление ключа доступа EXTR","Ваш ключ быстрого доступа был обновлён: <a href='http://futbolkaprint.com.ua/admin/home/extr/".$extr."'>".$extr."</a>");				
				return true;
			}
		}
		return false;
		...
		if(is_string($part))
		{
			switch($part)
			{
				...
				#die($sessions->getData("admin_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6)));
					$result=!($sessions->isDeath("admin_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6)));
					break;
				case 'client':
					$result=!($sessions->isDeath("user_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6)));
					break;
			}
		}
		return $result;
	...
	{
		$when=$GLOBALS['sessions']->whenDie("admin_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6));
		return $when/60;
	}
	...
	public function dropSession($part)
	{
		...
		$session=$part."_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6);
	...
	public function proceedClientAuthorize($email,$password)
	{
		...
		if(trim($email)!='' || trim($password)!='')
		{
			...
			if($check['status']=="ok")
			{
				...
				{
					if($sessions->registerData("user_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6),true,array(time(),3600*2)))
					{
	...
	public function proceedAdminAuthorize($data,$key)
	{
		...
		if(is_array($data) && $tools->checkValues($data,array('a'))){
			...
			if($database->checkRowExists("users",array("pin"=>$pin,"status"=>"on"))){
				if(!$database->isError())
				{
					$d=$sessions->registerData("admin_auth_".substr(md5($_SERVER['REMOTE_ADDR']),0,6),uniqid('i',true),array(time(),3600*2));
