...
if(isset($_POST['make']))
{
	...
	{
		if(!isset($_FILES['files']) || !isset($data['paths']) || count($_FILES['files'])!=count($data['paths']))
		{
			$errors->appendJSError("Ошибка во время проверки файлового списка!");
		}else
		{
			$xziparch=new xziparch();
			$arch=$xziparch->makeArch();
			$pack=$xziparch->makeArch();
			$root=$arch->make_dir('/');
			for($i=0;$i<count($_FILES['files']);$i++)
			{
				$xziparch->add($pack,$_FILES['files'][$i]['tmp_name'],$data['paths'][$i],false,$root);
			}
			$pack_data=$xziparch->saveArch($pack);
